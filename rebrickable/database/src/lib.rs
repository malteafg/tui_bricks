...
impl LocalDB {
    pub fn new<P: AsRef<Path>>(
        ...
        for rec in get_csv_reader(parts_path).unwrap().deserialize() {
            ...
            let category = categories.get_mut(&rec.part_cat_id).unwrap();
            category.parts.insert(rec.part_num.clone());
            ...
            parts.insert(
                ...
                Part {
                    ...
                    category_name: category.category_record.name.clone(),
                },
            );
        }
        ...
        for rec in get_csv_reader(colors_path).unwrap().deserialize() {
            ...
            colors.insert(rec.id, Color { color_record: rec });
        }
        ...
        for rec in get_csv_reader(elements_path).unwrap().deserialize() {
            ...
            let color_name = colors.get(&rec.color_id).unwrap().color_record.name.clone();
...
impl Default for LocalDB {
    fn default() -> Self {
        ...
        let mut categories_path = PathBuf::data_dir();
        categories_path.push("part_categories.csv");
...
//         assert_eq!(*color.color_record.id, -1);
//         assert_eq!(color.color_record.name, "[Unknown]".into());
//         assert_eq!(color.color_record.rgb, "0033B2");
//         assert_eq!(color.color_record.is_trans, false);
//         assert_eq!(color.color_record.num_parts, 20);
//         assert_eq!(color.color_record.num_sets, 5);
//         assert_eq!(color.color_record.y1, Some(2000));
//         assert_eq!(color.color_record.y2, Some(2012));

//         let color = &database.colors.get(&1.into()).unwrap();
//         assert_eq!(*color.color_record.id, 1);
//         assert_eq!(color.color_record.name, "Blue".into());
//         assert_eq!(color.color_record.rgb, "0055BF");
//         assert_eq!(color.color_record.is_trans, false);
//         assert_eq!(color.color_record.num_parts, 193056);
//         assert_eq!(color.color_record.num_sets, 46595);
//         assert_eq!(color.color_record.y1, Some(1949));
//         assert_eq!(color.color_record.y2, Some(2025));
//     }
...
//         assert_eq!(*element.element_record.element_id, 302123);
//         assert_eq!(*element.element_record.part_num, "3021");
//         assert_eq!(*element.element_record.color_id, 1);
//         assert_eq!(element.element_record.design_id, Some(3021));

//         let element = &database.elements.get(&407028.into()).unwrap();
//         assert_eq!(*element.element_record.element_id, 407028);
//         assert_eq!(*element.element_record.part_num, "4070");
//         assert_eq!(*element.element_record.color_id, 2);
//         assert_eq!(element.element_record.design_id, None);
//     }
